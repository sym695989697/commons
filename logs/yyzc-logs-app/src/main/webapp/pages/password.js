...
var Password=function(){
   ...
   this.modifyUrl=root+'/userJson/modifyPassword.action';
   this.logoutUrl=root + '/timeout.jsp';
};
...
Password.prototype = {
	...
	init:function(){
		...
		if(!isModifyPasswd){
			...
			$("#commitNewPassDiv").css('padding-left','140px');
			$("#logoutDiv").hide();
		}
  ...
  //当用户放开键盘或密码输入框失去焦点时,根据不同的级别显示不同的颜色
   ...
   logoutSystem:function(){
    ...
    commitUpdatePass: function(){   
      ...
      JAjax(password.modifyUrl,{'id':userId,'oldPasswd':hex_md5(oldPass),'newPasswd':hex_md5(newPass)}, 'json', function(data){
    	  debugger;
    	  if('success' == data){
   			 JSWK.clewBox('密码修改成功！','clew_ico_succ',1000);  
   			 setTimeout(function(){
