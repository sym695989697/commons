        ...
        loadData: function (loadDataParm)
        {
            ...
            if (p.dataAction == "server")
            {
                if (p.usePager)
                {
                    ...
                    param1[p.pageParmName] = p.newPage;
                    param1[p.pagesizeParmName] = p.pageSize;
                }
                if (p.sortName)
                {
                    param.push({ name: p.sortnameParmName, value: p.sortName });
                    param.push({ name: p.sortorderParmName, value: p.sortOrder });
                    param1[p.sortnameParmName] = p.sortName;
                    param1[p.sortorderParmName] = p.sortOrder;
                }
            };
            ...
            {
            	if(p.contentType){
            		g.loadServerData(JSON.stringify(param1), clause);
            	}else{
