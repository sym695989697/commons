...
public class ParameterConver {
		...
		for(Field f : fields){
			try{
				if(f.getAnnotation(claz) != null ){
					Field destField = dest.getClass().getDeclaredField(String.valueOf(f.getAnnotation(claz).getClass().getMethod("value", null).invoke(f.getAnnotation(claz), null)));
					if(destField != null){
	 ...
	 * @throws Exception
	 */
	private static Object getValByAttr(Field sourceField,Field targetField,Object sourceObj,Method sourceMethod) throws Exception {
	 ...
	 * @throws Exception
	 */
	public static void getValueFromObjectToSelf(Object source, Object dest, Class claz) throws Exception {
		List<Field> fields = new ArrayList<Field>();
		fields.addAll(Arrays.asList(source.getClass().getDeclaredFields()));
		for(Field f : fields){
			try{
	 ...
	 * @throws Exception
	 */
	public static void setValueOfField(Object object, String fieldName, Object value) throws Exception{
		PropertyDescriptor pd = new PropertyDescriptor(fieldName, object.getClass());
	...
	public static final Object paramToExampleExtended(DynamicSqlParameter dsp,
			Object newExample) throws ClassNotFoundException,
	...
	public static final Object paramToExampleExtendedCriteria(
			DynamicSqlParameter dsp, Object newExample)
			throws ClassNotFoundException, IllegalArgumentException,
			IllegalAccessException, InvocationTargetException {
		if (dsp == null && newExample == null)
			return null;

		Object criteria = ParameterConver.publicCall(newExample, "createCriteria",
				new Class[] {}, new Object[] {});
		if (criteria == null)
			throw new IllegalArgumentException(
					"newExample is not Ibatis Example parameter.");
		if (dsp == null)
			return criteria;

		String exaName = newExample.getClass().getName();
		String beanName = (exaName.endsWith("Example")) ? exaName.substring(0,
				exaName.length() - 7)
				: ((exaName.endsWith("ExampleExtended")) ? exaName.substring(0,
						exaName.length() - 15) : null);
		if (beanName == null)
		...
		if (orderField != null && !"".equals(orderField)) {
			...
			if (m != null) {
				...
				ParameterConver.publicCall(newExample, "setOrderByClause",
		...
		if (end != 0) {
			ParameterConver.publicCall(newExample, "setSkipNum",
					new Class[] { int.class },
					new Object[] { Integer.valueOf(start) });
			ParameterConver.publicCall(newExample, "setEndNum",
					new Class[] { int.class },
	...
	public static final Object publicCall(Object This, String method,
	...
	private static void andCriteria(Object criteria, Class beanClass, Map<String, String> params , String operator ) throws IllegalArgumentException, IllegalAccessException, InvocationTargetException{
		if( params !=null){
			for( String key : params.keySet() ){
				...
				if( getM == null){
					LogFactory.getLog(ParameterConver.class).warn("当拼写查询条件时有无效属性", new IllegalArgumentException("没有找到对应的GET方法："+key)) ;
					continue;
				}
